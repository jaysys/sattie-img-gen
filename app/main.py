...
class UplinkCommandRequest(BaseModel):
    ...
    generation_mode: GenerationMode = Field(
        ...
        description="Simulator-only optional field. Select INTERNAL or EXTERNAL image generation.",
    )
    external_map_source: ExternalMapSource = Field(
        default=ExternalMapSource.OSM,
        description="Simulator-only optional field for EXTERNAL mode. Current supported source: OSM.",
    )
    external_map_zoom: int = Field(
        default=19,
        ge=1,
        le=19,
        description="Simulator-only optional field for EXTERNAL mode. Map zoom level (1-19).",
    )
...
def fetch_tile_osm(zoom: int, x: int, y: int) -> Image.Image:
    ...
    req = url_request.Request(
        url,
        headers={
            "User-Agent": "satti-sim/0.2 (+https://localhost; contact: local-dev)",
        },
    )

# Satellite Img Gen Simuator Console 와이어프레임 설계서

- 문서명: Satellite Img Gen Simuator 검증 콘솔 와이어프레임 설계서
- 화면 타이틀: Satellite Img Gen Simuator Console
- 프로젝트: satti-sim
...
- Header: API Base URL, API Key, `Apply URL`, `Clear Images`, `Clear Logs`
...
```text
+----------------------------------------------------------------------------------+
| Header: K-Satellite Simulator Console                                            |
| [Base URL] [x-api-key] [Apply URL] [Clear Images] [Clear Logs]                  |
+----------------------------------------------------------------------------------+
...
- `Seed Initial Mock Satellites` 버튼 (`POST /seed/mock-satellites`)
- `Create Satellite` 폼
...
- `Completed Image Links` 테이블
  - 컬럼: Command ID, Satellite Name, State, Image Created At, Download
...
2. Satellites에서 `Seed Initial Mock Satellites` 실행
3. Uplink에서 유효 파라미터 입력 후 `Send Uplink`
4. 자동 전환된 Commands Monitor에서 상태 완료(`DOWNLINK_READY`) 확인
5. 다운로드 링크 검증
6. `Clear Images` 실행 후 링크 테이블 정리 상태 확인
7. Scenarios에서 오류 케이스 회귀 확인
